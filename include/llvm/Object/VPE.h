...
public:
  VPEObjectFile(MemoryBufferRef Object, std::error_code &EC);
  ...
  std::error_code getSymbol(uint32_t Index,
                            const vpe_symbol_type *&Res) const {
    if (Index >= getNumberOfSymbols())
      return object_error::parse_failed;

    Res = reinterpret_cast<vpe_symbol_type *>(getSymbolTable()) + Index;
    return std::error_code();
  }
  Expected<VPESymbolRef> getSymbol(uint32_t index) const {
    if (SymbolTable16) {
      const vpe_symbol16 *Symb = nullptr;
      if (std::error_code EC = getSymbol(index, Symb))
        return errorCodeToError(EC);
      return VPESymbolRef(Symb);
    }
    if (SymbolTable32) {
      const vpe_symbol32 *Symb = nullptr;
      if (std::error_code EC = getSymbol(index, Symb))
        return errorCodeToError(EC);
      return VPESymbolRef(Symb);
    }
  ...
  std::error_code getSymbolName(VPESymbolRef Symbol, StringRef &Res) const;
  std::error_code getSymbolName(const vpe_symbol_generic *Symbol,
                                StringRef &Res) const;
  ...
  std::error_code getSectionName(const vpe_section *Sec, StringRef &Res) const;
  uint64_t getSectionSize(const vpe_section *Sec) const;
  std::error_code getSectionContents(const vpe_section *Sec,
